const BLAST_SEQ_PATH = '/Users/vgs417/blast/seq/';
const BLAST_DATABASE_PATH = '/Users/vgs417/blast/db/';
const DATABASE_NAME_COI = 'bold_COI5P_99_consensus_2025_10_17.fasta' // 'bold_COI5P_99_consensus_2024_07_19.fasta' 'bold_COI5P_99_consensus_2024_01_06.fasta'; 
const DATABASE_NAME_ITS =  'sh_general_release_dynamic_s_all_25.07.2023.fasta'; // 'sh_general_release_dynamic_s_all_29.11.2022.fasta'; //'sh_general_release_dynamic_s_04.02.2020.fasta';
const DATABASE_NAME_16S =  'gtdb_ssu_reps_r214.fasta'; // 'gtdb_ssu_reps_r207.fasta'; //'gtdb_ssu_reps_r202.fasta'; // 'gtdb_ssu_reps_r95.fasta';
const DATABASE_NAME_12S =  'mitofish.12S.Mar2025.fasta'//'mitofish.12S.Dec2023.fasta'; //'mitofish.12S.May2023.fasta'
const DATABASE_NAME_18S = 'pr2_version_5.0.0_merged.fasta';
const DATABASE_NAME_RBCL = 'diat.database_v15_2.fasta';
const MATCH_THRESHOLD_ITS = 99; // 98.5;
const MATCH_THRESHOLD_COI = 99;
const MATCH_THRESHOLD_16S = 99;
const MATCH_THRESHOLD_12S = 99;
const MATCH_THRESHOLD_18S = 99;
const MATCH_THRESHOLD_RBCL = 99;
const MATCH_CLOSE_THRESHOLD = 90;
//const SUPPORTED_MARKERS = ['its', 'coi', 'co1', '16s', '12s', '18s', 'rbcl', '12S_mt12S_Eukaryotes_MIDORI2'.toLowerCase()];
const MAX_TARGET_SEQS = 5;
const LIMIT_MAX_TARGET_SEQS = 100;
const MINIMUM_QUERY_COVER =  50;// 80; // Minimum percentage of subject that is included in alignment
const NUM_THREADS = 1; // NUM_THREADS * NUM_CONCURRENT_PROCESSES should be equal to the number of cores available for this service
const NUM_CONCURRENT_PROCESSES = 10;
const CACHE_CONCURRENCY  = 50;

const env = process.env.NODE_ENV || 'local';
console.log('ENV: ' + env);


const HBASE = {
  hosts: ['c8n3.gbif.org', 'c8n7.gbif.org', 'c8n2.gbif.org'],
  port: 31995,
  tableName: 'blast_cache'
};

const DEFAULT_DATABASES = {'its': DATABASE_NAME_ITS, 'coi': DATABASE_NAME_COI, '16s': DATABASE_NAME_16S, '12s': DATABASE_NAME_12S, '18s': DATABASE_NAME_18S, 'rbcl': DATABASE_NAME_RBCL }
// mt12S_Eukaryotes_MIDORI2_UNIQ_NUC_SP_GB268_srRNA_BLAST_SeqID.fasta
const DATABASES = {...DEFAULT_DATABASES, 
  '12s_mt12s_eukaryotes_midori2': 'mt12S_Eukaryotes_MIDORI2_UNIQ_NUC_SP_GB268_srRNA_BLAST_SeqID.fasta',
  '16s_mt16s_eukaryotes_midori2': 'mt16S_Eukaryotes_MIDORI2_UNIQ_NUC_SP_GB268_lrRNA_BLAST_SeqID.fasta',
  'cytb_eukaryotes_midori2': 'Cytb_Eukaryotes_MIDORI2_UNIQ_NUC_SP_GB268_Cytb_BLAST_SeqID.fasta',
  '12s_nbdl': '12S_NBDL-MT-RNR1-e85f7a93142c5cf3_SeqID.fasta',
  '16s_nbdl': '16S_NBDL-MT-RNR2-cf376e15b7a71a3f_SeqID.fasta',
  'coi_nbdl': 'COI_NBDL-MT-CO1-f7e72089516344ba_SeqID.fasta',
  'cytb_nbdl': 'CYTB_NBDL-MT-CYB-f85bbc9846fbe32d_SeqID.fasta'

};
const config = {
  local: {
    BLAST_SEQ_PATH: BLAST_SEQ_PATH,
    BLAST_DATABASE_PATH: BLAST_DATABASE_PATH,
    DATABASE_NAME: DATABASES,
    MATCH_THRESHOLD: 99,//{'ITS': MATCH_THRESHOLD_ITS, 'COI': MATCH_THRESHOLD_COI, '16S': MATCH_THRESHOLD_16S, '12S': MATCH_THRESHOLD_12S, '18S': MATCH_THRESHOLD_18S, 'RBCL': MATCH_THRESHOLD_RBCL},
    MATCH_CLOSE_THRESHOLD: 90,//{'ITS': MATCH_CLOSE_THRESHOLD, 'COI': MATCH_CLOSE_THRESHOLD, '16S': MATCH_CLOSE_THRESHOLD, '12S': MATCH_CLOSE_THRESHOLD, '18S': MATCH_CLOSE_THRESHOLD, 'RBCL': MATCH_CLOSE_THRESHOLD},
    SUPPORTED_MARKERS: Object.keys(DATABASES),
    MAX_TARGET_SEQS: MAX_TARGET_SEQS,
    LIMIT_MAX_TARGET_SEQS: LIMIT_MAX_TARGET_SEQS,
    MINIMUM_QUERY_COVER: MINIMUM_QUERY_COVER,
    NUM_THREADS: NUM_THREADS,
    NUM_CONCURRENT_PROCESSES: NUM_CONCURRENT_PROCESSES,
    EXPRESS_PORT: 9001,
    HBASE: HBASE,
    CACHE_CONCURRENCY: CACHE_CONCURRENCY
  },
  production: {
    BLAST_SEQ_PATH: '/home/tsjeppesen/seq/',
    BLAST_DATABASE_PATH: '/home/tsjeppesen/',
    DATABASE_NAME: DATABASES,
    MATCH_THRESHOLD: 99,//{'ITS': MATCH_THRESHOLD_ITS, 'COI': MATCH_THRESHOLD_COI, '16S': MATCH_THRESHOLD_16S, '12S': MATCH_THRESHOLD_12S, '18S': MATCH_THRESHOLD_18S, 'RBCL': MATCH_THRESHOLD_RBCL},
    MATCH_CLOSE_THRESHOLD: 90,//{'ITS': MATCH_CLOSE_THRESHOLD, 'COI': MATCH_CLOSE_THRESHOLD, '16S': MATCH_CLOSE_THRESHOLD, '12S': MATCH_CLOSE_THRESHOLD, '18S': MATCH_CLOSE_THRESHOLD, 'RBCL': MATCH_CLOSE_THRESHOLD},
    SUPPORTED_MARKERS: Object.keys(DATABASES),
    MAX_TARGET_SEQS: MAX_TARGET_SEQS,
    LIMIT_MAX_TARGET_SEQS: LIMIT_MAX_TARGET_SEQS,
    MINIMUM_QUERY_COVER: MINIMUM_QUERY_COVER,
    NUM_THREADS: NUM_THREADS,
    NUM_CONCURRENT_PROCESSES: NUM_CONCURRENT_PROCESSES,
    EXPRESS_PORT: 80,
    HBASE: HBASE,
    CACHE_CONCURRENCY: CACHE_CONCURRENCY
  }
};

module.exports = config[env];
